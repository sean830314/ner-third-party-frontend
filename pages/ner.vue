                    ...
                    <el-tab-pane label="Result" name="Result">
                        <el-table :data="tableData" stripe style="width: 100%">
                            ...
                            <el-table-column label="Organization" width="700">
                                <div v-if="scope.row.organization.length > 0" slot-scope="scope">
                                    <div v-for="(organization, index) in scope.row.organization" :key="`${scope.row.organization}-example-${index}`">
                                        <span style="margin-right: 18px; color: red; font-size: 18px;">{{ index + 1 }}. {{ organization }}</span>
                                    </div>   
                                </div>
                            </el-table-column>
                            <el-table-column label="Address"  width="700">
                                 <div v-if="scope.row.address.length > 0" slot-scope="scope">
                                    <div v-for="(address, index) in scope.row.address" :key="`${scope.row.address}-example-${index}`">
                                        <span style="margin-right: 18px; color: red; font-size: 18px;">{{ index + 1 }}. {{ address }}</span>
                                    </div>
                                </div>
                            </el-table-column>
                            <el-table-column label="Date"  width="180">
                                <div v-if="scope.row.date.length > 0" slot-scope="scope">
                                    <div v-for="(date, index) in scope.row.date" :key="`${scope.row.date}-example-${index}`">
                                        <span style="margin-right: 18px; color: red; font-size: 18px;">{{ index + 1 }}. {{ date }}</span>
                                    </div>
                                </div>
                            </el-table-column>
    ...
    export default {
        ...
        data() {
            return {
                ...
                rules: {
                    paragraph: [
                        { required: true, message: 'please input a paragraph', trigger: 'blur' }
                    ],
                    model: [
                        { required: true, message: 'please choose a model type', trigger: 'change' }
                    ]
                },
                tableData: []
            };
        ...
        methods: {
            submitForm(formName) {
                ...
                if (valid) {
                    ...
                    try {
                        var responseData = []
                        const formData = {model: this.$data.ruleForm.model, paragraph: this.$data.ruleForm.paragraph}
